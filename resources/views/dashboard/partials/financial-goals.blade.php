<div class="card">
    <div
        ...
        <a href="{{ route('tujuan-keuangan.index') }}"
            class="btn btn-sm btn-primary px-3 d-flex align-items-center gap-1">
        ...
        @if (($financialGoalsActiveCount ?? 0) <= 0)
            <div class="text-center py-4">
                ...
                <p class="text-muted mb-0" style="font-size: 0.82rem;">
                    {{ __('Create a goal to start tracking progress here.') }}</p>
            </div>
        @else
            <div class="text-center mb-4">
                <h2 class="fw-bold mb-0 text-primary {{ $uiStyle === 'milenial' ? 'fw-extrabold' : '' }}"
            ...
            <div class="progress-clean mb-4">
                <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar"
                    ...
                    aria-valuenow="{{ $financialGoalsOverallPercent ?? 0 }}" aria-valuemin="0" aria-valuemax="100">
                </div>
            </div>
            ...
            <div class="row g-3 text-center mb-4">
                <div class="col-6">
                    <div
                        class="p-3 rounded-0 {{ $uiStyle === 'milenial' ? 'bg-primary bg-opacity-5' : 'stat-card-inner' }}">
                        <p class="section-label mb-1">{{ __('Collected') }}</p>
                        <h6 class="fw-bold mb-0 text-dark" id="goals-total-collected" style="font-size: 0.88rem;">
                            {{ $financialGoalsTotalCollectedView ?? 'Rp 0' }}</h6>
                    </div>
                </div>
                <div class="col-6">
                    <div class="p-3 rounded-0 stat-card-inner">
                        <p class="section-label mb-1">{{ __('Target') }}</p>
                        <h6 class="fw-bold mb-0 text-dark" id="goals-total-target" style="font-size: 0.88rem;">
                            {{ $financialGoalsTotalTargetView ?? 'Rp 0' }}</h6>
                    </div>
                </div>
            </div>
            ...
            @if (!empty($financialGoalsNextDue))
                <p class="text-muted text-center mb-4" style="font-size: 0.82rem;">
                    ...
                    {{ __('Next deadline') }}:
                    <strong
                        class="text-dark">{{ \Carbon\Carbon::parse($financialGoalsNextDue)->translatedFormat('d M Y') }}</strong>
                </p>
            @endif
                            ...
                            <div class="d-flex justify-content-between align-items-start gap-2 mb-2">
                                <div class="flex-grow-1">
                                    ...
                                    <p class="text-muted mb-0 mt-1" style="font-size: 0.78rem;">
                                        {{ __('Collected') }}: <span
                                            class="fw-semibold text-dark">{{ $goal['collectedView'] ?? 'Rp 0' }}</span>
                                        <span class="mx-1">/</span>
                                        {{ __('Target') }}: <span
                                            class="fw-semibold text-dark">{{ $goal['targetView'] ?? 'Rp 0' }}</span>
                                        @if ($due)
                                            <span class="mx-1">•</span>
                                            {{ __('Due') }}: <span
                                                class="fw-semibold text-dark">{{ \Carbon\Carbon::parse($due)->translatedFormat('d M Y') }}</span>
                                        @endif
                                    </p>
                                </div>
                                <div class="text-end">
