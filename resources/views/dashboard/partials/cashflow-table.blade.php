            ...
            <tr>
                ...
                <td class="text-end border-0">
                    <span class="text-success fw-bold">Rp
                        {{ number_format((float) $row->total_pemasukan, 0, ',', '.') }}</span>
                </td>
                <td class="text-end border-0">
                    <span class="text-danger fw-bold">Rp
                        {{ number_format((float) $row->total_pengeluaran, 0, ',', '.') }}</span>
                </td>
                <td class="text-end fw-bold border-0 {{ $uiStyle === 'milenial' ? 'pe-4' : '' }}">
                    <div class="{{ $row->selisih >= 0 ? 'text-dark' : 'text-danger' }}">
                        ...
                        @if ($row->selisih < 0)
                            <span
                                class="badge {{ $uiStyle === 'milenial' ? 'm-badge-modern' : '' }} bg-danger text-white ms-1">Defisit</span>
                        @elseif($row->selisih < 1000000)
                            <span
                                class="badge {{ $uiStyle === 'milenial' ? 'm-badge-modern' : '' }} bg-warning text-dark ms-1">Tipis</span>
                        @else
                            <span
                                class="badge {{ $uiStyle === 'milenial' ? 'm-badge-modern' : '' }} bg-success text-white ms-1">Aman</span>
                        @endif
                    </div>
                </td>
            </tr>
