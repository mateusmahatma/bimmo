...
@section('container')
    ...
    <section class="section">
                ...
                <div class="card mb-4">
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3">
                            <div>
                                <h5 class="card-title mb-1 fw-bold text-dark opacity-75" style="font-size: 1.1rem;">
                                    {{ __('Total Emergency Fund') }}</h5>
                                <p class="text-muted small mb-0">
                                    {{ __('Prepared funds for your unexpected financial needs.') }}</p>
                            </div>
                            <div class="text-end">
                                <h2 class="balance-amount mb-0" id="totalDanaDarurat">Rp
                                    {{ number_format($totalDanaDarurat, 0, ',', '.') }}</h2>
                            </div>
                        </div>
                        ...
                        <div class="mt-2 pt-3 border-top">
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                ...
                                <span class="text-dark small fw-bold"><span
                                        id="targetPercentage">{{ $percentage }}</span>%</span>
                            </div>
                            <div class="progress bg-light" style="height: 10px;">
                                <div class="progress-bar bg-danger" id="progressBar" role="progressbar"
                                    style="width: {{ $percentage }}%;" aria-valuenow="{{ $percentage }}"
                                    aria-valuemin="0" aria-valuemax="100"></div>
                            </div>
                            <div class="d-flex justify-content-between mt-2">
                                <small class="text-muted">{{ __('Target') }}: <span class="text-dark fw-semibold"
                                        id="targetDanaDaruratAmount">Rp
                                        {{ number_format($targetDanaDarurat, 0, ',', '.') }}</span></small>
                                <small
                                    class="text-muted italic small text-end">{{ __('Balanced for your security') }}</small>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('dana-darurat.create', ['type' => 1]) }}"
                                class="btn btn-success btn-sm px-3 shadow-sm">
                                <i class="bi bi-arrow-down-circle me-1"></i> {{ __('Deposit') }}
                            </a>
                            <a href="{{ route('dana-darurat.create', ['type' => 2]) }}"
                                class="btn btn-warning btn-sm px-3 shadow-sm text-dark">
                            ...
                            <table id="danaDaruratTable" class="table table-hover align-middle mb-0" style="width:100%">
                                <thead class="bg-light">
                                    <tr style="border-bottom: 2px solid #edf2f9;">
                                        ...
                                        <th style="width: 5%;"
                                            class="text-secondary small text-uppercase fw-bold py-3 text-center">
                                            {{ __('No') }}</th>
                                        ...
                                        <th class="text-secondary small text-uppercase fw-bold py-3 text-center">
                                            {{ __('Updated') }}</th>
                                        <th style="width: 10%;"
                                            class="text-center text-secondary small text-uppercase fw-bold py-3">
                                            {{ __('Action') }}</th>
                                    </tr>
                                </thead>
                                <tbody></tbody>
        ...
        <div class="fab-container d-md-none"
            style="position: fixed; bottom: 20px; right: 20px; z-index: 1050; display: flex; flex-direction: column; gap: 10px;">
            <a href="{{ route('dana-darurat.create', ['type' => 2]) }}"
                class="btn btn-warning rounded-circle shadow d-flex align-items-center justify-content-center text-dark"
                style="width: 56px; height: 56px;" title="{{ __('Withdrawal') }}">
                <i class="bi bi-arrow-up fs-4"></i>
            </a>
            <a href="{{ route('dana-darurat.create', ['type' => 1]) }}"
                class="btn btn-success rounded-circle shadow d-flex align-items-center justify-content-center"
                style="width: 56px; height: 56px;" title="{{ __('Deposit') }}">
