                ...
                <div class="card">
                    <div class="card-body">
                        <div class="card-header bg-white border-bottom py-3">
                            <h5 class="card-title mb-0 fw-bold text-dark" style="font-size: 1.1rem;">
                                {{ __('Add Emergency Fund Data') }}</h5>
                            <p class="text-muted small mb-0 mt-1">
                                {{ __('Fill in the details for your emergency fund transaction.') }}</p>
                        </div>
                    </div>
                    <div class="card-body p-4">
                        ...
                        <form action="{{ route('dana-darurat.store') }}" method="POST">
                            ...
                            <div class="mb-4">
                                <label for="tgl_transaksi_dana_darurat"
                                    class="form-label fw-bold small text-uppercase text-muted">{{ __('Transaction Date') }}
                                    <span class="text-danger">*</span></label>
                                <input name="tgl_transaksi_dana_darurat" type="date" class="form-control form-control-lg"
                                    id="tgl_transaksi_dana_darurat"
                                    value="{{ old('tgl_transaksi_dana_darurat', date('Y-m-d')) }}" required>
                            </div>

                            <div class="row g-3 mb-4">
                                <div class="col-md-6">
                                    <label for="jenis_transaksi_dana_darurat"
                                        class="form-label fw-bold small text-uppercase text-muted">{{ __('Transaction Type') }}
                                        <span class="text-danger">*</span></label>
                                    <select name="jenis_transaksi_dana_darurat" id="jenis_transaksi_dana_darurat"
                                        class="form-select form-select-lg" required>
                                        <option value="">-- {{ __('Select Status') }} --</option>
                                        <option value="1"
                                            {{ old('jenis_transaksi_dana_darurat', isset($type) ? $type : '') == '1' ? 'selected' : '' }}>
                                            {{ __('Deposit') }}</option>
                                        <option value="2"
                                            {{ old('jenis_transaksi_dana_darurat', isset($type) ? $type : '') == '2' ? 'selected' : '' }}>
                                            {{ __('Withdrawal') }}</option>
                                    </select>
                                    <div id="infoSaldo"
                                        class="mt-2 p-2 bg-light rounded border-start border-4 border-warning small {{ old('jenis_transaksi_dana_darurat', isset($type) ? $type : '') == '2' ? '' : 'd-none' }}">
                                        <i class="bi bi-info-circle me-1 text-warning"></i> {{ __('Current balance') }}:
                                        <span class="fw-bold text-dark">Rp
                                            {{ number_format($totalDanaDarurat, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <label for="nominal_dana_darurat"
                                        class="form-label fw-bold small text-uppercase text-muted">{{ __('Amount (Rp)') }}
                                        <span class="text-danger">*</span></label>
                                    <div class="input-group input-group-lg">
                                        <span class="input-group-text bg-light text-muted fw-bold">Rp</span>
                                        <input type="text" class="form-control" name="nominal_dana_darurat"
                                            id="nominal_dana_darurat" placeholder="0"
                                            value="{{ old('nominal_dana_darurat') }}" inputmode="numeric" required>
                                    </div>
                                    <div id="containerTarikSemua"
                                        class="mt-2 {{ old('jenis_transaksi_dana_darurat', isset($type) ? $type : '') == '2' ? '' : 'd-none' }}">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="tarikSemuaSaldo"
                                                style="cursor: pointer;">
                                            <label class="form-check-label small" for="tarikSemuaSaldo"
                                                style="cursor: pointer;">
                            ...
                            <div class="mb-4">
                                <label for="keterangan"
                                    class="form-label fw-bold small text-uppercase text-muted">{{ __('Note') }}</label>
                                <textarea class="form-control" name="keterangan" id="keterangan" rows="3"
                                    placeholder="{{ __('Add a note...') }}"></textarea>
                            </div>

                            <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-4">
                                <a href="{{ route('dana-darurat.index') }}"
                                    class="btn btn-light px-4">{{ __('Cancel') }}</a>
