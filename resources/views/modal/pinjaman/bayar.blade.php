    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 15px;">
            <div class="modal-header border-bottom-0 pb-0">
                <h5 class="modal-title fw-bold text-dark" id="bayarModalLabel">{{ __('Pay Loan') }}</h5>
            ...
            <div class="modal-body py-4">
                <form id="bayarForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        <label for="jumlah_bayar" class="form-label small fw-bold text-muted text-uppercase">{{ __('Payment Amount') }}</label>
                    ...
                    <div class="mb-3">
                        <label for="tgl_bayar" class="form-label small fw-bold text-muted text-uppercase">{{ __('Payment Date') }}</label>
                        <input type="date" id="tgl_bayar" name="tgl_bayar" class="form-control rounded-3" required>
                    </div>
                    <div class="mb-0">
                        <label for="bukti_bayar" class="form-label small fw-bold text-muted text-uppercase">{{ __('Payment Proof (Optional)') }}</label>
                        ...
                        <div id="current_file_container" class="mt-2 d-none p-2 bg-light rounded-3 d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center">
                                ...
                                <span class="small text-muted">{{ __('Current proof attached') }}</span>
                            </div>
                            <a id="current_file_link" href="#" target="_blank" class="btn btn-sm btn-link text-primary p-0 text-decoration-none small fw-bold">{{ __('View File') }}</a>
                        </div>
                    </div>
                    <div class="modal-footer border-top-0 pt-4 px-0 pb-0">
                        ...
                        <button type="submit" class="btn btn-primary rounded-pill px-4 btn-color shadow-sm">
                            <i class="bi bi-check2-circle me-1"></i> {{ __('Pay Loan') }}
                        </button>
                    </div>
                </form>
            </div>
