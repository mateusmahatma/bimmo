                    ...
                    <div class="card-body p-4 d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            <h5 class="card-title mb-1 fw-bold text-dark opacity-75" style="font-size: 1.1rem;">
                                {{ __('Asset Summary') }}</h5>
                            <p class="text-muted small mb-0">{{ __('Overview of your physical and digital belongings.') }}
                            </p>
                        </div>
                        <div class="text-end">
                ...
                <div class="card p-3">
                    <div class="d-flex flex-wrap gap-2 align-items-center justify-content-between">
                        ...
                        <div class="btn-add-desktop">
                            <a href="{{ route('aset.create') }}" class="btn btn-primary btn-sm px-3">
                        ...
                        <div class="table-responsive p-3">
                            <table id="asetTable" class="table table-hover align-middle mb-0" style="width:100%">
                                <thead class="bg-light">
                                    <tr>
                                        <th class="text-secondary small text-uppercase fw-bold py-3">{{ __('Code') }}
                                        </th>
                                        <th class="text-secondary small text-uppercase fw-bold py-3">{{ __('Asset Name') }}
                                        </th>
                                        <th class="text-secondary small text-uppercase fw-bold py-3">{{ __('Category') }}
                                        </th>
                                        <th class="text-secondary small text-uppercase fw-bold py-3">{{ __('Location') }}
                                        </th>
                                        <th class="text-secondary small text-uppercase fw-bold py-3">
                                            {{ __('Purchase Date') }}</th>
                                        <th class="text-secondary small text-uppercase fw-bold py-3">
                                            {{ __('Current Value (Book)') }}</th>
                                        <th class="text-secondary small text-uppercase fw-bold py-3">{{ __('Condition') }}
                                        </th>
                ...
                window.confirmAction({
                    ...
                    onConfirm: async () => {
                        try {
                            ...
                            showToast('{{ __('Asset has been deleted successfully.') }}',
                                'success');
                            table.ajax.reload(null, false);
                        } catch (e) {
                            showToast(
                                '{{ __('Something went wrong while deleting the asset.') }}',
                                'danger');
                        }
                    }
                });
