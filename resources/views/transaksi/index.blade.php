    ...
    <div class="row">
            ...
            <div class="card card-dashboard h-100 d-flex flex-column justify-content-center">
                <div class="card-body py-4">
                    <div class="d-flex align-items-center mb-2">
                        <div class="card-icon transaksi-card-icon transaksi-card-icon--success rounded-circle d-flex align-items-center justify-content-center bg-success-light text-success me-3">
        ...
        <div class="col-md-4 mb-4">
            <div class="card card-dashboard h-100 d-flex flex-column justify-content-center">
                <div class="card-body py-4">
                    <div class="d-flex align-items-center mb-2">
                       <div class="card-icon transaksi-card-icon transaksi-card-icon--danger rounded-circle d-flex align-items-center justify-content-center bg-danger-light text-danger me-3">
             ...
             <div class="card card-dashboard h-100 d-flex flex-column justify-content-center">
                <div class="card-body py-4">
                    <div class="d-flex align-items-center">
                        <div class="card-icon transaksi-card-icon transaksi-card-icon--primary rounded-circle d-flex align-items-center justify-content-center bg-primary-light text-primary me-3">
            ...
            <div class="card card-dashboard">
                <div class="card-body py-3">
                    <div class="d-flex flex-column align-items-start text-start">
                        <h6 class="fw-bold text-dark text-uppercase mb-1 transaksi-period-heading">{{ __('Period Expense') }}</h6>
                        <span class="badge bg-light text-secondary border fw-normal rounded-pill px-2 py-1 mb-2 transaksi-badge-date-range" id="avg-date-range">
                            <i class="bi bi-calendar3 me-1"></i> {{ $dateRange }}
                        </span>

                        <div class="d-flex align-items-center">
                             <div class="text-start">
                                <h4 class="mb-0 fw-bold text-dark transaksi-avg-daily-value" id="avg-daily">Rp {{ number_format($avgDailyPengeluaran, 0, ',', '.') }}</h4>
                                <span class="text-muted small text-uppercase transaksi-daily-avg-label">{{ __('Daily Average') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-12">
            <div class="card card-dashboard border-0 shadow-sm transaksi-main-card">
                <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
                    <div>
                        <h5 class="card-title mb-0 fw-bold text-dark transaksi-section-title">{{ __('Transaction Dates') }}</h5>
                        <p class="text-muted small mb-0 mt-1 transaksi-section-desc">{{ __('Click a date to view and add transactions.') }}</p>
                    </div>
                </div>
...
<form action="{{ route('transaksi.index') }}" method="GET" class="transaksi-toolbar mb-4 pt-3">
    ...
    <div class="toolbar-row toolbar-row--primary">
        ...
        <div class="toolbar-daterange">
            <input type="hidden" name="start_date" id="transaksi_start_date" value="{{ request('start_date') }}">
            <input type="hidden" name="end_date" id="transaksi_end_date" value="{{ request('end_date') }}">
            <div id="transaksiDaterange"
                ...
                <span class="transaksi-daterange-label flex-grow-1 text-truncate small">{{ $dateRange }}</span>
        ...
        <div class="toolbar-actions d-none d-lg-flex align-items-center gap-2">
            {{-- Filter --}}
            <button class="btn btn-outline-secondary btn-sm rounded-pill px-3 d-flex align-items-center gap-1"
            ...
            {{-- Export --}}
            <div class="dropdown">
                <button class="btn btn-outline-success btn-sm dropdown-toggle rounded-pill px-3 d-flex align-items-center gap-1"
                    type="button" data-bs-toggle="dropdown">
                    <i class="bi bi-download"></i>
                    <span>{{ __('Export') }}</span>
                </button>
                <ul class="dropdown-menu shadow border-0 rounded-3">
                    <li><a class="dropdown-item no-loader" id="btnExportExcel" href="{{ route('transaksi.export.excel', request()->all()) }}">
                        <i class="bi bi-file-earmark-excel me-2 text-success"></i> Excel</a></li>
                    <li><a class="dropdown-item no-loader" id="btnExportPdf" href="{{ route('transaksi.export.pdf', request()->all()) }}">
                    ...
                    <li><a class="dropdown-item" id="btnExportEmail" href="#"
                        ...
                        <i class="bi bi-envelope me-2 text-primary"></i> {{ __('Export to Email') }}</a></li>
                </ul>
            </div>
    ...
    <div class="toolbar-row toolbar-row--shortcuts">
        <div class="toolbar-quick-dates">
            <a href="{{ route('transaksi.index', array_merge(request()->query(), ['start_date' => date('Y-m-d'), 'end_date' => date('Y-m-d')])) }}"
                class="btn btn-outline-secondary btn-xs rounded-pill" title="{{ __('Today') }}">{{ __('Today') }}</a>
            <a href="{{ route('transaksi.index', array_merge(request()->query(), ['start_date' => date('Y-m-d', strtotime('-1 day')), 'end_date' => date('Y-m-d', strtotime('-1 day'))])) }}"
                class="btn btn-outline-secondary btn-xs rounded-pill" title="{{ __('Yesterday') }}">{{ __('Yesterday') }}</a>
            <a href="{{ route('transaksi.index', array_merge(request()->query(), ['start_date' => date('Y-m-01'), 'end_date' => date('Y-m-t')])) }}"
                class="btn btn-outline-secondary btn-xs rounded-pill" title="{{ __('This Month') }}">{{ __('This Month') }}</a>
            <a href="{{ route('transaksi.index', array_merge(request()->query(), ['start_date' => date('Y-m-01', strtotime('-1 month')), 'end_date' => date('Y-m-t', strtotime('-1 month'))])) }}"
                class="btn btn-outline-secondary btn-xs rounded-pill" title="{{ __('Last Month') }}">{{ __('Last Month') }}</a>
            <a href="{{ route('transaksi.index', array_merge(request()->query(), ['start_date' => date('Y-01-01'), 'end_date' => date('Y-12-31')])) }}"
                class="btn btn-outline-secondary btn-xs rounded-pill" title="{{ __('This Year') }}">{{ __('This Year') }}</a>
            <a href="{{ route('transaksi.index', array_merge(request()->query(), ['start_date' => date('Y-01-01', strtotime('-1 year')), 'end_date' => date('Y-12-31', strtotime('-1 year'))])) }}"
                class="btn btn-outline-secondary btn-xs rounded-pill" title="{{ __('Last Year') }}">{{ __('Last Year') }}</a>
        </div>

        {{-- Mobile action buttons --}}
        <div class="toolbar-mobile-actions d-flex d-lg-none align-items-center gap-2 ms-auto">
            {{-- Filter --}}
            <button class="btn btn-outline-secondary btn-xs rounded-pill d-flex align-items-center gap-1"
                type="button" data-bs-toggle="collapse" data-bs-target="#filterCollapse">
                <i class="bi bi-sliders"></i>
            ...
            {{-- Export dropdown --}}
            <div class="dropdown">
                <button class="btn btn-outline-success btn-xs rounded-pill dropdown-toggle d-flex align-items-center gap-1"
                    type="button" data-bs-toggle="dropdown">
                    <i class="bi bi-download"></i>
                    <span class="d-none d-sm-inline">{{ __('Export') }}</span>
                </button>
                <ul class="dropdown-menu dropdown-menu-end shadow border-0 rounded-3">
                    <li><a class="dropdown-item no-loader" id="btnExportExcelMobile" href="{{ route('transaksi.export.excel', request()->all()) }}">
                        <i class="bi bi-file-earmark-excel me-2 text-success"></i> Excel</a></li>
                    <li><a class="dropdown-item no-loader" id="btnExportPdfMobile" href="{{ route('transaksi.export.pdf', request()->all()) }}">
                        <i class="bi bi-file-earmark-pdf me-2 text-danger"></i> PDF</a></li>
                    <li><hr class="dropdown-divider"></li>
                    ...
                    <div class="collapse mb-4 {{ request()->hasAny(['pemasukan', 'pengeluaran']) ? 'show' : '' }}" id="filterCollapse">
                        <div class="card card-body bg-light border-0 p-4 rounded-4">
                            <form method="GET" class="row g-3">
                                ...
                                <input type="hidden" name="start_date" id="transaksi_filter_form_start" value="{{ request('start_date') }}">
                                <input type="hidden" name="end_date" id="transaksi_filter_form_end" value="{{ request('end_date') }}">
...
@push('scripts')
<script id="transaksi-page-config" type="application/json">@json($transaksiPageConfig)</script>
