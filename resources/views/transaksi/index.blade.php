...
<script>
    (function() {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Guard: bail if any required library is missing
            if (
                typeof $ === 'undefined' ||
            ...
            try {
                ...
                const $label = $picker.find('.transaksi-daterange-label');

                // ── Initial dates ─────────────────────────────────────
                const initialStart = (startInput && startInput.value) ?
                    moment(startInput.value, valueFormat) :
                    moment().startOf('month');

                const initialEnd = (endInput && endInput.value) ?
                    moment(endInput.value, valueFormat) :
                    moment().endOf('month');

                // ── Preset ranges ─────────────────────────────────────
                const ranges = {
                    '{{ __("Today") }}': [moment().startOf('day'), moment().endOf('day')],
                    '{{ __("Yesterday") }}': [moment().subtract(1, 'day').startOf('day'), moment().subtract(1, 'day').endOf('day')],
                ...
                $picker.daterangepicker({
                    ...
                    function(start, end) {
                        ...
                        // Submit the toolbar form to apply the filter
                ...
                if ($label.length) {
                    $label.text(
                        initialStart.format(displayFormat) + ' – ' + initialEnd.format(displayFormat)
                    );
                }

            } catch (e) {
                // Silent fail – do not break the UI
                console.error('[transaksi] daterangepicker init error:', e);
            }
        });
    }());
</script>
