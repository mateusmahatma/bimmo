                ...
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('transaksi.store') }}" method="POST" id="transactionForm">
                        ...
                        @php
                            ...
                            $initialMode =
                                $hasOldIncome && $hasOldExpense ? 'both' : ($hasOldExpense ? 'expense' : 'income');
                        @endphp
                        ...
                        <div class="row g-4 mb-4">
                            ...
                            <div class="col-lg-6">
                                ...
                                <input type="date" class="form-control" id="tgl_transaksi" name="tgl_transaksi"
                                    value="{{ old('tgl_transaksi', $defaultDate ?? date('Y-m-d')) }}" required>
                            </div>

                            <!-- Wallet Selection -->
                            <div class="col-lg-6">
                                <label for="dompet_id"
                                    class="form-label fw-bold small text-uppercase text-muted">{{ __('Select Wallet') }}</label>
                                <select class="form-select" id="dompet_id" name="dompet_id">
                                    <option value="">- {{ __('Select Wallet') }} -</option>
                                    @foreach ($dompet as $d)
                                        <option value="{{ $d->id }}">{{ $d->nama }} (Rp
                                            {{ number_format((float) $d->saldo, 0, ',', '.') }})</option>
                                    @endforeach
                                </select>
                                <div class="form-text small">
                                    {{ __('Transaction will affect the balance of the selected wallet.') }}</div>
                            </div>
                        </div>
                        ...
                        <div class="row g-4 mb-4" id="transaksiTypePanelsRow">
                            <!-- Income Section -->
                            <div class="col-md-6 collapse {{ $initialMode !== 'expense' ? 'show' : '' }}"
                                id="pemasukanSection">
                                <div class="transaksi-panel transaksi-panel--income p-4 border bg-white mb-3"
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-5">
                                            ...
                                            <div class="input-group">
                                                <span
                                                    class="input-group-text bg-success text-white fw-bold border-0">Rp</span>
                                                <input type="number" id="nominal_pemasukan" name="nominal_pemasukan"
                                                    class="form-control fw-bold text-success border-start-0"
                                                    placeholder="0">
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <!-- Expense Section -->
                            <div class="col-md-6 collapse {{ $initialMode !== 'income' ? 'show' : '' }}"
                                id="pengeluaranSection">
                                <div class="transaksi-panel transaksi-panel--expense p-4 border bg-white"
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-5">
                                            ...
                                            <div class="input-group">
                                                <span
                                                    class="input-group-text bg-danger text-white fw-bold border-0">Rp</span>
                                                <input type="number" id="nominal" name="nominal"
                                                    class="form-control fw-bold text-danger border-start-0"
                                                    placeholder="0">
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="mb-4">
                            <label for="keterangan"
                                class="form-label fw-bold small text-uppercase text-muted">{{ __('Description / Notes') }}</label>
                            <textarea class="form-control" id="keterangan" name="keterangan" rows="3"
                                placeholder="{{ __('Additional details about this transaction...') }}"></textarea>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const setMode = (mode) => {
                ...
                const incomeCollapse = bootstrap.Collapse.getOrCreateInstance(pemasukanSectionEl, {
                    toggle: false
                });
                const expenseCollapse = bootstrap.Collapse.getOrCreateInstance(pengeluaranSectionEl, {
                    toggle: false
                });
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (!hasIncome && !hasExpense) {
                    alert(
                        "{{ __('Please fill in at least one transaction type (Income or Expense) completely with an amount greater than 0.') }}"
                    );
                    return;
                }
                    ...
                    .then(async response => {
                        ...
                        if (!response.ok) {
                            let errorMsg = data?.message ||
                                '{{ __('Terjadi kesalahan pada sistem.') }}';
                            ...
                            if (response.status === 422 && data?.errors) {
                                ...
                                errorMsg =
                                    `<ul class="mb-0 mt-2 ps-3 small text-start">${errorMsg}</ul>`;
                            }
            ...
            if (typeof TomSelect !== 'undefined') {
                if (document.getElementById('pemasukan')) incSelect = new TomSelect('#pemasukan', {
                    allowEmptyOption: true,
                    placeholder: '- {{ __('Select Income') }} -'
                });
                if (document.getElementById('pengeluaran')) expSelect = new TomSelect('#pengeluaran', {
                    allowEmptyOption: true,
                    placeholder: '- {{ __('Select Expense') }} -'
                });
                if (document.getElementById('barang_id')) new TomSelect('#barang_id', {
                    allowEmptyOption: true,
