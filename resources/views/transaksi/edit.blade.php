                    ...
                    <div class="card-body p-4">
                        ...
                        <form action="{{ route('transaksi.update', $hashId) }}" method="POST">
                            ...
                            @php
                                ...
                                $initialMode =
                                    $hasIncome && $hasExpense ? 'both' : ($hasExpense ? 'expense' : 'income');
                            @endphp
                                    ...
                                    <select class="form-select" id="dompet_id" name="dompet_id">
                                        ...
                                        @foreach ($dompet as $d)
                                            <option value="{{ $d->id }}"
                                                ...
                                                {{ $d->nama }} (Rp
                                                {{ number_format((float) $d->saldo, 0, ',', '.') }})
                                            </option>
                                        @endforeach
                                    </select>
                                    <div class="form-text small">
                                        {{ __('Changing the wallet will revert the balance update from the old wallet.') }}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row g-4 mb-4" id="transaksiTypePanelsRow">
                                ...
                                <div class="col-md-6 collapse {{ $initialMode !== 'expense' ? 'show' : '' }}"
                                    id="pemasukanSection">
                                    <div class="transaksi-panel transaksi-panel--income p-4 border bg-white income-expense-card"
                                        style="border-top: 5px solid #198754 !important;">
                                        <h6 class="text-success fw-bold mb-3"><i class="bi bi-arrow-down-circle me-2"></i>
                                            {{ __('Income') }}</h6>
                                ...
                                <div class="col-md-6 collapse {{ $initialMode !== 'income' ? 'show' : '' }}"
                                    ...
                                    <div class="transaksi-panel transaksi-panel--expense p-4 border bg-white income-expense-card"
                                        style="border-top: 5px solid #dc3545 !important;">
                                        <h6 class="text-danger fw-bold mb-3"><i class="bi bi-arrow-up-circle me-2"></i>
                                            {{ __('Expense') }}</h6>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea class="form-control" id="keterangan" name="keterangan" rows="3"
                                    placeholder="{{ __('Additional details about this transaction...') }}">{{ old('keterangan', $transaksi->keterangan) }}</textarea>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (typeof TomSelect !== 'undefined') {
                if (document.getElementById('pemasukan')) new TomSelect('#pemasukan', {
                    allowEmptyOption: true,
                    placeholder: '- {{ __('Select Income') }} -'
                });
                if (document.getElementById('pengeluaran')) new TomSelect('#pengeluaran', {
                    allowEmptyOption: true,
