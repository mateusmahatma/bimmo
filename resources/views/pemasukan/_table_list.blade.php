            ...
            <tr style="border-bottom: 2px solid #edf2f9;">
                ...
                <th class="text-secondary small text-uppercase fw-bold py-3">
                    <a href="{{ $sortLink('nama') }}"
                        ...
                        {{ __('Income Name') }} @if ($currentSort === 'nama')
        ...
        <tbody>
            @forelse ($pemasukan as $row)
                <tr>
                    ...
                    <td class="text-center text-secondary fw-medium">{{ $loop->iteration + ($pemasukan->currentPage() - 1) * $pemasukan->perPage() }}</td>
                    <td class="fw-semibold text-dark">{{ $row->nama }}</td>
                    <td class="text-center text-muted small">
                        <span style="font-family: 'Consolas', monospace;">{{ $row->created_at->format('Y-m-d H:i:s') }}</span>
                    </td>
                    <td class="text-center text-muted small">
                        <span style="font-family: 'Consolas', monospace;">{{ $row->updated_at->format('Y-m-d H:i:s') }}</span>
                    </td>
                    <td class="text-center">
                        @include('pemasukan.tombol', ['request' => $row])
                    </td>
                </tr>
            @empty
                <tr>
