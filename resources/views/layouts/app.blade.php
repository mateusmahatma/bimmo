    ...
    <script>
        (function() {
            window.applyTheme = (theme, persist = true) => {
                const isDark = theme === 'dark' || (theme === 'auto' && window.matchMedia(
                    '(prefers-color-scheme: dark)').matches);
                document.documentElement.setAttribute('data-bs-theme', isDark ? 'dark' : 'light');

                const favicon = document.getElementById('favicon');
                if (favicon) {
                    favicon.setAttribute('href', isDark ? "{{ asset('img/bimmo_dark_favicon.png') }}" :
                        "{{ asset('img/bimmo_favicon.png') }}");
                }
...
<body class="{{ auth()->check() ? 'ui-style-' . (auth()->user()->ui_style ?? 'corporate') : '' }}"
    ...
    <script>
        ...
        window.confirmAction = function(options) {
            ...
            modalElement.querySelector('.modal-title-inject').textContent = options.title ||
                '{{ __('Are you sure?') }}';
            modalElement.querySelector('.modal-text-inject').textContent = options.text ||
                '{{ __('Deleted data cannot be recovered!') }}';
