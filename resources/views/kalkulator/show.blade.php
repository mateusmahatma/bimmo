...
@push('css')
    ...
    <style>
        ...
        [data-bs-theme="dark"] .burn-gauge-bar {
    ...
    <section class="section">
        <div class="row">
                        ...
                        <div class="row g-4">
                            <div class="col-md-6">
                                <table class="table table-borderless table-sm">
                                    <tbody>
                                        <tr>
                                            <td class="text-muted small text-uppercase fw-bold" style="width: 140px;">
                                                {{ __('Budget Name') }}</td>
                                        ...
                                        <tr>
                                            <td class="text-muted small text-uppercase fw-bold">{{ __('Expense Type') }}
                                            </td>
                                            <td>
                                                <ul class="list-unstyled mb-0">
                                                    ...
                                                    @if (count($namaPengeluaran) > 5)
                                                        <li class="text-muted fst-italic ms-3 small">
                                                            +{{ count($namaPengeluaran) - 5 }} {{ __('others') }}</li>
                                                    @endif
                                                </ul>
                                            </td>
                                        </tr>
                                ...
                                <div class="card bg-light h-100">
                                    <div class="card-body">
                                        <h6 class="text-muted small text-uppercase fw-bold mb-3">
                                            {{ __('Financial Summary') }}</h6>
                                        <div class="d-flex justify-content-between mb-2">
                                            <span>{{ __('Budget Amount') }}:</span>
                                            <span class="fw-bold">Rp
                                                {{ number_format($HasilProsesAnggaran->nominal_anggaran, 0, ',', '.') }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between mb-2">
                                            <span>{{ __('Used') }}:</span>
                                            <span class="fw-bold text-danger">Rp
                                                {{ number_format($HasilProsesAnggaran->anggaran_yang_digunakan, 0, ',', '.') }}</span>
                                        </div>
                                        <hr>
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <div class="text-end">
                                                ...
                                                @if ($sisa < 0)
                                                    <span
                                                        class="badge bg-danger-subtle text-danger small">{{ __('Over Budget') }}</span>
                                                @else
                                                    <span
                                                        class="badge bg-success-subtle text-success small">{{ __('Within Budget') }}</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
            ...
            @if ($burnRate)
                ...
                <div class="col-lg-12">
                    <div
                        class="card mb-4 burn-rate-card {{ $burnRate['alert_triggered'] ? 'alert-danger-card' : 'alert-safe-card' }}">
                        <div class="card-body p-4">

                            {{-- Header --}}
                            <div class="d-flex align-items-center gap-3 mb-4">
                                <div class="d-flex align-items-center justify-content-center {{ $burnRate['alert_triggered'] ? 'bg-danger-subtle' : 'bg-success-subtle' }}"
                                ...
                                <div>
                                    <h5
                                        class="fw-bold mb-0 {{ $burnRate['alert_triggered'] ? 'text-danger' : 'text-success' }}">
                                ...
                                <div class="ms-auto text-end d-none d-md-block">
                                    <span
                                        class="badge {{ $burnRate['alert_triggered'] ? 'bg-danger' : 'bg-success' }} rounded-pill px-3 py-2"
                                        style="font-size:.8rem;">
                                        {{ number_format($burnRate['spent_percentage'], 1) }}% {{ __('used') }}
                                    </span>
                                    <div class="text-muted small mt-1">{{ __('after') }} {{ $burnRate['days_elapsed'] }}
                                        {{ __('days elapsed') }}</div>
                                </div>
                            </div>
                            ...
                            <div class="burn-gauge-wrap mb-1">
                                <div class="burn-gauge-bar">
                                    <div class="burn-gauge-fill"
                                        style="width:{{ $fillPct }}%; background:{{ $fillColor }}"></div>
                                </div>
                                <div class="burn-ideal-marker" style="left:{{ $idealPct }}%"
                                    title="{{ __('Ideal pace') }}"></div>
                            </div>
                            <div class="d-flex justify-content-between small text-muted mb-4">
                                <span>{{ __('Day') }} {{ $burnRate['days_elapsed'] }}</span>
                                <span class="opacity-75"><i class="bi bi-bar-chart-line me-1"></i>{{ __('Ideal:') }}
                                    {{ number_format($burnRate['ideal_percentage'], 1) }}%</span>
                            ...
                            <div class="row g-3 mb-4">
                                <div class="col-6 col-md-3">
                                    <div class="burn-stat-box">
                                        <div class="text-muted small text-uppercase fw-bold mb-1" style="font-size:.7rem;">
                                            {{ __('Spend / Day') }}</div>
                                        <div
                                            class="fw-bold fs-6 {{ $burnRate['alert_triggered'] ? 'text-danger' : 'text-success' }}">
                                            Rp {{ number_format($burnRate['daily_rate'], 0, ',', '.') }}</div>
                                    </div>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="burn-stat-box">
                                        <div class="text-muted small text-uppercase fw-bold mb-1" style="font-size:.7rem;">
                                            {{ __('Days Remaining') }}</div>
                                        <div class="fw-bold fs-6">{{ $burnRate['days_remaining'] }} {{ __('days') }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="burn-stat-box">
                                        <div class="text-muted small text-uppercase fw-bold mb-1" style="font-size:.7rem;">
                                            {{ __('Projected Total Spend') }}</div>
                                        <div
                                            class="fw-bold fs-6 {{ $burnRate['projected_total'] > $burnRate['total_budget'] ? 'text-danger' : 'text-success' }}">
                                            Rp {{ number_format($burnRate['projected_total'], 0, ',', '.') }}</div>
                                    </div>
                                </div>
                                <div class="col-6 col-md-3">
                                    <div class="burn-stat-box">
                                        <div class="text-muted small text-uppercase fw-bold mb-1" style="font-size:.7rem;">
                                            {{ __('Budget Runs Out In') }}</div>
                                        <div
                                            class="fw-bold fs-6 {{ $burnRate['alert_triggered'] ? 'text-danger' : 'text-success' }}">
                            ...
                            {{-- Alert / Safe Message --}}
                            @if ($burnRate['alert_triggered'])
                                <div class="alert mb-0" style="background:rgba(239,68,68,.08);">
                                    <div class="d-flex align-items-start gap-2">
                                        <i class="bi bi-exclamation-triangle-fill text-danger mt-1 flex-shrink-0"></i>
                                        <div>
                                            <strong
                                                class="text-danger">{{ __('Your spending rate is too fast!') }}</strong>
                                            <p class="mb-0 small text-muted mt-1">
                                                {{ __('After') }} <strong>{{ $burnRate['days_elapsed'] }}</strong>
                                                {{ __('days, you have already spent') }}
                                                <strong>{{ number_format($burnRate['spent_percentage'], 1) }}%</strong>
                                                {{ __('of your budget.') }}
                            ...
                            @else
                                <div class="alert border-0 mb-0" style="background:rgba(34,197,94,.08);">
                                    <div class="d-flex align-items-start gap-2">
                                        ...
                                        <div>
                                            <strong
                                                class="text-success">{{ __('Your spending is under control.') }}</strong>
                                            <p class="mb-0 small text-muted mt-1">
                                                {{ __('At your current pace, your budget is projected to') }}
                                                <strong
                                                    class="text-success">{{ __('last through the end of the period') }}</strong>.
                                                {{ __('Keep it up!') }}
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <div class="search-bar" style="min-width: 200px;">
                                <div class="input-group input-group-sm">
                                    ...
                                    <input type="text" id="detailSearch"
                                        ...
                                        style="font-size: 0.8rem;" placeholder="{{ __('Search transactions...') }}">
                                </div>
                            </div>
                        </div>
