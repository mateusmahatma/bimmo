...
@section('container')
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
                        <div class="card-body">
                            <h5 class="card-title mb-0 fw-bold text-dark"
                                style="font-size: 1.1rem; letter-spacing: -0.01em;">{{ __('Budget Process History') }}</h5>
                            <p class="text-muted small mb-0 mt-1">{{ __('List of your budget calculation history.') }}</p>
                        </div>
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-outline-success btn-sm d-none rounded-pill"
                                id="btnBulkSync" style="padding: 2px 10px; font-size: 0.75rem;">
                                <i class="bi bi-arrow-repeat me-1"></i> {{ __('Sync Selected') }}
                            </button>
                            <button type="button" class="btn btn-outline-danger btn-sm d-none rounded-pill"
                                id="btnBulkDelete" style="padding: 2px 10px; font-size: 0.75rem;">
                            ...
                            <div class="search-bar" style="min-width: 200px;">
                                <div class="input-group input-group-sm">
                                    ...
                                    <input type="text" id="historySearch"
                                        ...
                                        style="font-size: 0.8rem;" placeholder="{{ __('Search history...') }}">
                                </div>
                            </div>

                            <div class="d-flex align-items-center gap-2">
                                <label for="historyPeriodeFilter" class="text-muted small mb-0">{{ __('Periode') }}</label>
                                <select id="historyPeriodeFilter" class="form-select form-select-sm"
                                    style="min-width: 260px;">
                                    <option value="">{{ __('Semua') }}</option>
                                    @foreach ($periods ?? [] as $p)
                                        <option value="{{ $p->id_periode_anggaran }}"
                                            {{ (string) request('id_periode_anggaran') === (string) $p->id_periode_anggaran ? 'selected' : '' }}>
    ...
    <div class="modal fade" id="prosesPeriodeModal" tabindex="-1" aria-labelledby="prosesPeriodeModalLabel"
        aria-hidden="true">
