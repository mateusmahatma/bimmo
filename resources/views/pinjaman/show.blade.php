            ...
            <div class="col-lg-12 mb-4">
                <div class="card h-100">
                    <div class="card-body p-4">
                        <h5 class="card-title fw-bold mb-4 text-dark" style="font-size: 1.1rem;">
                            {{ __('Loan Information') }}</h5>
                        <div class="row g-4">
                            <div class="col-md-6 border-start border-primary border-4 py-1 ps-3">
                                <label class="small text-muted text-uppercase fw-bold mb-1 d-block"
                                    style="letter-spacing: 0.5px;">{{ __('Loan Name') }}</label>
                            ...
                            <div class="col-md-6 border-start border-success border-4 py-1 ps-3">
                                <label class="small text-muted text-uppercase fw-bold mb-1 d-block"
                                    style="letter-spacing: 0.5px;">{{ __('Current Balance') }}</label>
                                <p class="fs-5 fw-bold text-primary mb-0">Rp
                                    {{ number_format($pinjaman->jumlah_pinjaman, 0, ',', '.') }}</p>
                            </div>
                            <div class="col-md-4">
                                <label
                                    class="small text-muted text-uppercase fw-bold mb-1 d-block">{{ __('Duration') }}</label>
                                <p class="mb-0 fw-semibold">{{ $pinjaman->jangka_waktu }} {{ __('Months') }}</p>
                            </div>
                            <div class="col-md-4">
                                <label
                                    class="small text-muted text-uppercase fw-bold mb-1 d-block">{{ __('Monthly Installment') }}</label>
                                <p class="mb-0 fw-semibold text-warning">Rp
                                    {{ number_format($pinjaman->nominal_angsuran, 0, ',', '.') }}</p>
                            </div>
                            <div class="col-md-4">
                                <label
                                    class="small text-muted text-uppercase fw-bold mb-1 d-block">{{ __('Status') }}</label>
                            ...
                            <div class="col-md-6 border-top pt-3 mt-3">
                                <label
                                    class="small text-muted text-uppercase fw-bold mb-1 d-block">{{ __('Start Date') }}</label>
                                <p class="mb-0 fw-semibold">
                                    {{ \Carbon\Carbon::parse($pinjaman->start_date)->format('d M Y') }}</p>
                            </div>
                            <div class="col-md-6 border-top pt-3 mt-3">
                                <label
                                    class="small text-muted text-uppercase fw-bold mb-1 d-block">{{ __('End Date') }}</label>
                                <p class="mb-0 fw-semibold">
                                    {{ \Carbon\Carbon::parse($pinjaman->end_date)->format('d M Y') }}</p>
                            </div>
                            @if ($pinjaman->keterangan)
                                <div class="col-12 border-top pt-3 mt-3">
                                    <label class="small text-muted text-uppercase fw-bold mb-1 d-block text-primary"
                                        style="letter-spacing: 0.5px;">{{ __('Keterangan') }}</label>
                    ...
                    <div class="card overflow-hidden">
                        <div class="card-body p-4">
                            <div class="d-flex align-items-center mb-4 pb-2 border-bottom">
                                ...
                                <div>
                                    <h5 class="card-title mb-0 fw-bold text-dark" style="font-size: 1.1rem;">
                                        {{ __('Future Installment Schedule') }}</h5>
                                    <p class="text-muted small mb-0">
                                        {{ __('Projected repayment metrics based on current balance.') }}</p>
                                </div>
                            </div>
                            ...
                            <div class="row g-4 mb-3">
                                <div class="col-md-6 col-lg-4">
                                    <div class="p-3 bg-light rounded-3 h-100">
                                        <label class="small text-muted text-uppercase fw-bold mb-1 d-block"
                                            style="letter-spacing: 0.5px; font-size: 0.7rem;">{{ __('Estimated Remaining') }}</label>
                                ...
                                <div class="col-md-6 col-lg-4">
                                    <div class="p-3 bg-light rounded-3 h-100">
                                        <label class="small text-muted text-uppercase fw-bold mb-1 d-block"
                                            style="letter-spacing: 0.5px; font-size: 0.7rem;">{{ __('Next Installment') }}</label>
                                        <h4 class="mb-0 fw-bold text-primary">Rp
                                            {{ number_format(min($pinjaman->nominal_angsuran, $pinjaman->jumlah_pinjaman), 0, ',', '.') }}
                                        </h4>
                                    </div>
                                </div>
                                <div class="col-lg-4 d-none d-lg-block">
                                    <div class="p-3 h-100 d-flex align-items-center justify-content-center text-center">
                                        <i class="bi bi-graph-down-arrow-fill text-primary opacity-25"
                            ...
                            <div class="d-flex align-items-center p-2 rounded-2"
                                style="background: rgba(65, 84, 241, 0.05);">
                                <i class="bi bi-info-circle text-primary me-2"></i>
                                <span
                                    class="small text-muted">{{ __('Note: This is an estimation based on your current balance and monthly installment amount.') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card">
                        <div class="card-body p-4">
                            <div class="d-flex align-items-center mb-4 pb-2 border-bottom">
                                ...
                                <div>
                                    <h5 class="card-title mb-0 fw-bold text-dark" style="font-size: 1.1rem;">
                                        {{ __('Simulasi Cicilan (Rencana Awal)') }}</h5>
                                    <p class="text-muted small mb-0">
                                        {{ __('Jadwal cicilan yang direncanakan saat pembuatan pinjaman.') }}</p>
                                </div>
                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($pinjaman->simulasi_cicilan as $simulasi)
                                            ...
                                            <tr>
                                                ...
                                                <td class="fw-bold text-end pe-4">Rp
                                                    {{ number_format($simulasi['nominal'], 0, ',', '.') }}</td>
                                                <td class="text-center">
                                                    @if ($is_paid)
                                                        <span
                                                            class="badge bg-success-light text-success px-3 py-2 rounded-pill">
                                                    ...
                                                    @elseif ($pinjaman->status === 'belum_lunas')
                                                        <button type="button"
                                                            ...
                                                            data-bs-toggle="modal" data-bs-target="#bayarModal"
                ...
                <div class="card">
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <div>
                                <h5 class="card-title mb-0 fw-bold text-dark" style="font-size: 1.1rem;">
                                    {{ __('Payment History') }}</h5>
                                <p class="text-muted small mb-0 mt-1">{{ __('Track all payments made for this Loan.') }}
                                </p>
                            </div>
                            @if ($pinjaman->status === 'belum_lunas')
                                ...
                                <tbody>
                                    @forelse ($pinjaman->bayar_pinjaman as $index => $bayar_pinjaman)
                                        <tr>
                                            ...
                                            <td data-label="{{ __('Amount') }}" class="fw-bold text-success">Rp
                                                {{ number_format($bayar_pinjaman->jumlah_bayar, 0, ',', '.') }}</td>
                                            <td data-label="{{ __('Date') }}">
                                                {{ \Carbon\Carbon::parse($bayar_pinjaman->tgl_bayar)->format('d M Y') }}
                                            </td>
                                            <td data-label="{{ __('Proof') }}" class="text-center">
                                                @if ($bayar_pinjaman->bukti_bayar)
                                                    <a href="{{ asset('storage/' . $bayar_pinjaman->bukti_bayar) }}"
                                                        target="_blank" class="btn btn-sm btn-outline-info"
                                                        data-bs-toggle="tooltip" title="{{ __('View Proof') }}"
                                            ...
                                            <td data-label="{{ __('Action') }}" class="text-center">
                                                <div class="d-flex justify-content-center gap-1">
                                                    <button type="button"
                                                        class="btn btn-outline-primary btn-sm edit-bayar"
                                                        data-id="{{ $bayar_pinjaman->id_bayar }}"
                                                        data-bs-toggle="tooltip" title="{{ __('Edit Payment') }}"
                                                        style="width: 32px; height: 32px; padding:0; display:flex; align-items:center; justify-content:center;">
                                                        <i class="bi bi-pencil"></i>
                                                    </button>
                                                    <form
                                                        action="{{ route('bayar-pinjaman.destroy', $bayar_pinjaman->id_bayar) }}"
                                                        method="POST" class="d-inline"
                                                        onsubmit="return confirm('{{ __('Are you sure you want to delete this payment record?') }}');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-outline-danger btn-sm"
                                                            data-bs-toggle="tooltip" title="{{ __('Delete Payment') }}"
                                                            style="width: 32px; height: 32px; padding:0; display:flex; align-items:center; justify-content:center;">
