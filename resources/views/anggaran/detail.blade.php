...
<div class="pagetitle mb-4">
    <h1 class="fw-bold mb-1">
        {{ __('Anggaran') }} — {{ $periode->nama_periode }}
    </h1>
    <nav>
        <ol class="breadcrumb mb-0">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('Dashboard') }}</a></li>
            <li class="breadcrumb-item"><a href="{{ route('anggaran.index') }}">{{ __('Anggaran') }}</a></li>
            <li class="breadcrumb-item active">{{ __('Detail') }}</li>
        </ol>
    </nav>
    <div class="text-muted small mt-1">
        {{ __('Periode:') }} {{ optional($periode->tanggal_mulai)->format('Y-m-d') }} — {{ optional($periode->tanggal_selesai)->format('Y-m-d') }}
    </div>
</div>
            ...
            <div class="card card-dashboard border-0 shadow-sm" style="border-radius: 12px;">
                <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button type="button" class="btn btn-primary btn-sm shadow-sm tombol-tambah-anggaran btn-add-desktop" style="padding: 2px 10px; font-size: 0.75rem;">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-4 pt-3">
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ route('anggaran.index') }}" class="btn btn-outline-secondary btn-sm rounded-pill" style="padding: 2px 10px; font-size: 0.75rem;">
...
@push('scripts')
<script>
    window.ANGGARAN_BASE_URL = @json($baseUrl);
</script>
