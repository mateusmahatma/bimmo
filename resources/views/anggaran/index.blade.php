            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
                        <div class="card-body">
                            <h5 class="card-title mb-0 fw-bold text-dark" style="letter-spacing: -0.01em;">
                                {{ __('List of Budget Periods') }}</h5>
                            ...
                            <table class="table table-hover align-middle mb-0">
                                <thead class="bg-light">
                                    <tr>
                                        ...
                                        <th class="text-secondary small fw-bold py-3 text-center">{{ __('Start Date') }}
                                        </th>
                                        <th class="text-secondary small fw-bold py-3 text-center">
                                            {{ __('Completion Date') }}</th>
                                        <th class="text-secondary small fw-bold py-3 text-center" style="width: 10%;">
                                            {{ __('Action') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <tr>
                                            <td class="fw-bold">
                                                <a class="text-decoration-none"
                                                    href="{{ route('anggaran.detail', $p->id_periode_anggaran) }}">
                                                    {{ $p->nama_periode }}
                                                </a>
                                            </td>
                                            <td class="text-center text-muted small">
                                                {{ optional($p->tanggal_mulai)->format('Y-m-d') }}</td>
                                            <td class="text-center text-muted small">
                                                {{ optional($p->tanggal_selesai)->format('Y-m-d') }}</td>
                                            <td class="text-center">
                                                <form method="POST"
                                                    action="{{ route('anggaran.destroy', $p->id_periode_anggaran) }}"
